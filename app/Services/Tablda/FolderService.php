...
class FolderService
{
     ...
     * @param $user_id
     */
    public function loadSettings(Folder $folder, $user_id) {
        ...
        $auth = app( AuthUserSingleton::class );
        $folder->_sub_tree = $auth->getMenuTreeFolder($folder->id);
    }
     ...
     * @param $name
     * @param $structure
     * @return array|\Vanguard\Models\Folder\Folder
     */
    ...
    public function updateFolder($folder_id, Array $data, $user_id) {
        return $this->folderRepository->updateFolder($folder_id, $data);
    }
     ...
     * @return mixed
     */
    public function deleteFolderWithSubs(int $folder_id) {
        ...
        return $this->folderRepository->deleteFolders($folder_ids);
    }
    ...
    public function moveFolder(Folder $folder, $new_parent_id, $user_id, int $position = null) {
        ...
        return $this->folderRepository->moveFolder($folder->id, $new_parent_id);
    }
    ...
    public function favoriteToggle($folder_id, $user_id, $favorite) {
        ...
        if ($folderFavorite) {
            $this->deleteFolderWithSubs($folderFavorite->id);
        }
