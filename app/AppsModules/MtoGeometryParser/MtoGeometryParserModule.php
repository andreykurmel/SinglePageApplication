...
namespace Vanguard\AppsModules\MtoGeometryParser;
...
class MtoGeometryParserModule
{
    ...
    public function __construct(Table $table, TableFieldLink $link)
    {
        $this->table = $table;
        $this->link = $link;
        $this->outputTable = $link->_mto_geom_output_table;
        $this->prefix = 'mtogeom_';
    ...
    public function parse(int $row_id): string
    {
        ...
        if ($this->link->mto_geom_remove_prev_rec) {
    ...
    protected function getContentFromPdf($row_id): array
    {
        ...
        $file = $repo->getSql($this->table->id, $this->link->mto_geom_doc_field_id, [$row_id])->first();
        copy(
            Storage::path('/public/'.$file->filepath.$file->filename),
            dirname(__FILE__).'/truePDFS_w_MTO_Geometry/1.pdf'
        );

        shell_exec('chmod 777 '.dirname(__FILE__).'/truePDFS_w_MTO_Geometry/1.pdf');
        shell_exec('python3 '.dirname(__FILE__).'/PDF_MTO_Geometry.py');

        try {
            $json = file_get_contents(dirname(__FILE__) . '/truePDFS_w_MTO_Geometry/1_Geometry.json');
