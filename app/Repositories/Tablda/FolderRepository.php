...
class FolderRepository
{
     ...
     * @return mixed
     */
    public function linkTable($table_id, $folder_id, $user_id, $type = 'table', $structure = 'private') {
    ...
    public function addSysFolderWithSysTables($folder_name, $user_id = null) {
        if ($folder_name == 'System') {
            ...
            $sys_tables = Table::whereIn('db_name', $this->service->support_tables)->get();
     ...
     * @return mixed
     */
    public function moveFolder($folder_id, $parent_id = 0) {
     ...
     * @return mixed
     */
    public function updateFolder($folder_id, Array $data) {
        return Folder::where('id', '=', $folder_id)
            ->update( collect($data)->only(['name', 'is_opened', 'icon_path'])->toArray() );
    }
