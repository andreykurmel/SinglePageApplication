        ...
        methods: {
            ...
            getTableData() {
                if (this.$root.user.see_view) {
                    return; //Favorites are temporal for MRV
                }
            ...
            massUpdatedRows(massTableRows) {
                Endpoints.massUpdateRows(this.tableMeta, massTableRows, this.newRequestParams()).then((data) => {
                    _.each(massTableRows, (row) => {
                    ...
                    eventBus.$emit('sync-list-view-update', massTableRows);
                });
            },
            deleteRow(tableRow) {
                Endpoints.deleteRow(this.tableMeta, tableRow).then((data) => {
                    eventBus.$emit('sync-list-view-delete', tableRow.id);
            ...
            syncFavoritesUpdateHandler(massTableRows) {
                _.each(massTableRows, (row_data) => {
                    let id = row_data ? row_data.id : null;
                    let oldrow = _.find(this.$root.favoriteTableRows, {id: id});
                    if (oldrow) {
                        this.$root.assignObject(row_data, oldrow);
                    }
                });
            },
            syncFavoritesDeleteHandler(rowId) {
                let idx = _.findIndex(this.$root.favoriteTableRows, {id: rowId});
                if (idx > -1) {
