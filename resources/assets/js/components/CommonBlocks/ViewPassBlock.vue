...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            checkPass() {
                axios.post('/ajax/table-view/lock-pass', {
                    tb_view_hash: this.$root.user.view_hash,
                    fld_view_hash: this.$root.user._is_folder_view,
                    app_view_hash: this.$root.user.view_all ? this.$root.user.view_all.hash : '',
                    pass: this.pass
                }).then(({ data }) => {
                    if (data.status) {
