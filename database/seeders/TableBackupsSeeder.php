...
class TableBackupsSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        $this->create('table_view_id', 'Data Range (View)', $table_sub, $user, 'String');
        $this->create('day', 'Frequency', $table_sub, $user, 'String');
        $this->create('overwrite', 'Overwritten', $table_sub, $user, 'Boolean', 0, '', '* If turned OFF, a "_yyyymmdd" will be appended to the end of saved files.');
        $this->create('root_folder', 'Subfolder', $table_sub, $user, 'String', 0, '', '* A subfolder will be added under "Dropbox/ Apps / TablDA / Autobackup" for Autobackup. If left empty,  the account`s username will be used for the subfolder.');
        $this->create('timezone', 'Timezone', $table_sub, $user, 'Timezone');
        $this->create('time', 'Time', $table_sub, $user, 'Time');
        $this->create('mysql', 'MySQL', $table_sub, $user, 'Boolean', 0);
        $this->create('csv', 'CSV', $table_sub, $user, 'Boolean', 0);
        $this->create('attach', 'Attachments', $table_sub, $user, 'Boolean', 0);
