...
class UserConnectonsAndCloudsSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        $this->create('name', 'Name', $table_sub, $user);
        $this->create('type', 'Provider', $table_sub, $user);
        $this->create('model', 'Model', $table_sub, $user);
        $this->create('air_type', 'Type', $table_sub, $user);
        $this->create('air_base', 'Base ID', $table_sub, $user);
        $this->create('jira_email', 'Email', $table_sub, $user);
        $this->create('key', 'Key', $table_sub, $user, 'String', 1);
        $this->create('auth_token', 'Token', $table_sub, $user);
        $this->create('twiml_app_id', 'TwiML App ID', $table_sub, $user);
        $this->create('twilio_phone', 'Phone', $table_sub, $user);
        $this->create('jira_host', 'Host', $table_sub, $user);
        ...
        $this->create('email_active', 'Status', $table_sub, $user, 'Boolean');
        //add right to Visitor
        $this->permissionsService->addSystemRights($table_sub->id, 1);
        $this->viewRepository->addSys($table_sub);
        ...
        $table_sub = Table::where('db_name', 'table_twilio_addon_settings')->first();
        if (!$table_sub) {
            $table_sub = Table::create([
                'is_system' => 1,
                'db_name' => 'table_twilio_addon_settings',
                'name' => 'Twilio Addon Settings',
                'user_id' => $user->id,
                'rows_per_page' => 100,
                'source' => 'scratch',
                'created_by' => $user->id,
        ...
        //headers for 'Email Addon Settings'
        $this->create('name', 'Name', $table_sub, $user, 'String', 1);
        $this->create('acc_twilio_key_id', 'Twilio Credential', $table_sub, $user, 'String');
        $this->create('description', 'Description', $table_sub, $user, 'String');
        $this->create('twilio_active', 'Status', $table_sub, $user, 'Boolean');
        //add right to Visitor
        $this->permissionsService->addSystemRights($table_sub->id, 1);
        $this->viewRepository->addSys($table_sub);
    }
